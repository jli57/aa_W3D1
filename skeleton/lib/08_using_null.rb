...
def teachers_and_depts
  ...
  execute(<<-SQL)
  SELECT 
    t.name, COALESCE(d.name, 'None') AS Department 
  FROM 
    teachers AS t 
  LEFT JOIN
    depts AS d 
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT 
    COUNT(name) as count_teachers, COUNT(mobile) AS count_mobile 
  FROM 
    teachers 
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  SELECT 
    d.name, COUNT(t.name) AS count_staff 
  FROM 
    depts AS d 
  LEFT JOIN 
    teachers as t 
